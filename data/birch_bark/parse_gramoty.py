...
def parse_gramoty(data: pd.DataFrame):
    ...
    # Aggregation
    grouped = data.groupby("Header").agg({
        'Full context': 'first',
        'Created': 'first',
        'Title': 'first',
    ...
    dataset = grouped[[
        'Header', 'Full context', 'time_period', 'target_year', 'year_start', 'year_end', 'Para context 1',
        'Para context 2', 'Para context 3'
    ]]
    dataset.columns = [
        'doc_id', 'text', 'time_period', 'avg_year', 'year_start', 'year_end', 'translation_ru', 'translation_en_1',
        'translation_en_2'
    ]
